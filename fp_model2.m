...
for turn=0:299
    ...
    for k=turn*10+2:turn*10+10
        ...
        Hr1(turn*10+1)=(Gcf(turn*10+1).*Cpc.*(Tra1(turn*10+1)-Trg2(turn*10+1))+(Go(turn*10+1)+Gro(turn*10+1)).*Cpo.*(Tra1(turn*10+1)-220)+Gw(turn*10+1).*Cpw.*(Tra1(turn*10+1)-200))./(Go(turn*10+1)+Gro(turn*10+1))*(-1);
        Hrd1(turn*10+1)=Hr1(turn*10+1);
        Hr2(turn*10+1)=((Tra2(turn*10+1)-Tra1(turn*10+1)).*((Go(turn*10+1)+Gro(turn*10+1)).*Cpo+Gw(turn*10+1).*Cpw+Gcf(turn*10+1).*Cpc))./(Go(turn*10+1)+Gro(turn*10+1))*(-1);
        Hrd2(turn*10+1)=Hr2(turn*10+1);

        Hr1(k)=((dense_o(1).*Vr(1).*Cpo+Gcf(k).*Vr(1).*dense_o(1)./(Go(k)+Gro(k)).*Cpc+2.*dense_L.*pi.*...
            L(1).*(r_in(1)+Th./2).*Th.*CpL*0.7+Gw(k).*dense_o(1).*Vr(1).*Cpw./(Go(k)+Gro(k))).*(Tra1(k)-Tra1(k-1))*60+...
            Gcf(k).*Cpc.*(Tra1(k)-Trg2(k))+(Go(k)+Gro(k)).*Cpo.*(Tra1(k)-220)+Gw(k).*Cpw.*(Tra1(k)-200))./(Go(k)+Gro(k)).*(-1);
        Hrd1(k)=0.2.*Hrd1(k-1)+(1-0.2).*((dense_o(1).*Vr(1).*Cpo+Gcf(k).*Vr(1).*dense_o(1)./(Go(k)+Gro(k)).*Cpc+2.*dense_L.*pi.*...
            L(1).*(r_in(1)+Th./2).*Th.*CpL*0.7+Gw(k).*dense_o(1).*Vr(1).*Cpw./(Go(k)+Gro(k))).*(Tra1(k)-Tra1(k-1))*60+...
            Gcf(k).*Cpc.*(Tra1(k)-Trg2(k))+(Go(k)+Gro(k)).*Cpo.*(Tra1(k)-220)+Gw(k).*Cpw.*(Tra1(k)-200))./(Go(k)+Gro(k)).*(-1);

        Hr2(k)=((dense_o(2).*Vr(2).*Cpo+Gcf(k).*Vr(2).*dense_o(2)./(Go(k)+Gro(k)).*Cpc+2.*dense_L.*pi.* ...
        ...
        Hrd2(k)=0.2.*Hrd2(k-1)+(1-0.2).*((dense_o(2).*Vr(2).*Cpo+Gcf(k).*Vr(2).*dense_o(2)./(Go(k)+Gro(k)).*Cpc+2.*dense_L.*pi.* ...
            L(2).*(r_in(2)+Th./2).*Th.*CpL*0.7+Gw(k).*dense_o(2).*Vr(2).*Cpw./(Go(k)+Gro(k))).*(Tra2(k)-Tra2(k-1))*60+...
            (Tra2(k)-Tra1(k)).*((Go(k)+Gro(k)).*Cpo+Gw(k).*Cpw+Gcf(k).*Cpc))./(Go(k)+Gro(k)).*(-1);
        %% 稳态
        %   Hr(k)=(Gcf(k).*Cpc.*(Tra1(k)-Trg2(k))+(Go(k)+Gro(k)).*Cpo.*(Tra1(k)-220)+Gw(k).*Cpw.*(Tra1(k)-200)+...
        %         (Tra2(k)-Tra1(k)).*((Go(k)+Gro(k)).*Cpo+Gw(k).*Cpw+Gcf(k).*Cpc))./(Go(k)+Gro(k)).*(-1); %积累+延提升管变化=反应热，三项同号
        %   Hrd(k)=0.2.*Hrd(k-1)+(1-0.2).*(Gcf(k).*Cpc.*(Tra1(k)-Trg2(k))+(Go(k)+Gro(k)).*Cpo.*(Tra1(k)-220)+Gw(k).*Cpw.*(Tra1(k)-200)+...
        %         (Tra2(k)-Tra1(k)).*((Go(k)+Gro(k)).*Cpo+Gw(k).*Cpw+Gcf(k).*Cpc))./(Go(k)+Gro(k)).*(-1);
    end
end
Gcf=Gcf';
